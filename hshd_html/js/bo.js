...
;(function(self) {	
	var canvas,
		...
		currentTransition = '慧声惠搭', 
		left, right, 
		layout = 0, 
		type = ['慧声惠搭', 'ovals', 'drop', 'ribbon','hehe'], 
		FPS = 60,
		...
		words = [ '慧声惠搭', 'ovals', 'drop', 'ribbon','hehe' ],
	...
	function createParticles() {
		...
		for(var quantity = 0, len = 200; quantity < len; quantity++) {
			...
			var color = colors.慧声惠搭[~~(Math.random() * colors.慧声惠搭.length)];
				...
				if(Math.round(particle.radius) === 0) {
					...
					switch(currentTransition) {
						...
						case '慧声惠搭':
							...
							particle.color = colors.慧声惠搭[~~(Math.random() * colors.慧声惠搭.length)];
